function [state_array, location_array, type_idx_array, frac_total_transcript, energy, s_i, time_out, time_P_cell, frac_P_count_vec, temp, transition_array] = WholeCell_2_final_ex3 (state_array, location_array, beta_array_cell, type_idx_array, frac_total_transcript, energy, s_i, time_in, time_P_cell, frac_P_count_vec, temp, transition_array)
    ...
    if ribo_movement == 1
        ...
        elseif sum(state_array==(prev_state-1))>0
            if mrna_diluted == 0 || (mrna_diluted == 1 && prev_location < mrna_to_be_deleted)
            ...
            elseif mrna_diluted == 1 && prev_location > mrna_to_be_deleted %then previous location now 
                %corresponds to one less mrna index because of backshift
                if sum(location_array(state_array==(prev_state-1))==prev_location-1) == 1 
                    vec = find(state_array==(prev_state-1));
                    rib = vec(location_array(state_array==(prev_state-1))==prev_location-1);
                    transition_array(rib+1) = beta_array_cell{type_idx_array(prev_location-1)}(prev_state-1);
                end
            end %else if previous_location==mrna_to_be_deleted: ribosomes already taken care of 
            %(they are all reset)
        end
