function [state_array, location_array, type_idx_array, frac_total_transcript, energy, s_i, time_out, time_P_cell, frac_P_count_vec, temp, transition_array] = Gillespie_STS_Prod_Rate_Multi_WholeCell (state_array, location_array, beta_array_cell, type_idx_array, frac_total_transcript, energy, s_i, time_in, time_P_cell, frac_P_count_vec, temp, transition_array)
    ...
    %elongation rate
    if energy < 10^5
        beta_array_cell{1} = beta_array_cell{1}(2:end)*(energy/(energy+1000));
        beta_array_cell{2} = beta_array_cell{2}(2:end)*(energy/(energy+1000));
        beta_array_cell{3} = beta_array_cell{3}(2:end)*(energy/(energy+1000));
    end
    ...
    if ribo_movement == 1
        ...
        if location_array(ribo)~=0
            ...
            if state_array(ribo) == length(beta_array_cell{type_idx_array(location_array(ribo))})
                ...
                transition_array(ribo+1) = 0; %need to zero this by definition, since ribo becomes free, so
                %no elongation, only initiation can happen!!
                ...
                if ~isequal(round(frac_total_transcript), total_transcript)
                    ...
                    for i = decreased_types
                        ...
                        if ~ismember(i, unoccupied_types)
                            ...
                            if max(unoccupied_mrna) > length(type_idx_array) || isempty(unoccupied_mrna)
                                disp('Indexing Error: unoccupied_mrna. Program paused, Press Ctrl-C to exit!');
                                pause;
                            end
        ...
        if prev_state == 2
            if prev_location==0
                disp('Logical Error: State 2 with Location 0. Program paused, Press Ctrl-C to exit!');
                pause;
            end
        ...
        if protein_diluted == 0
            ...
            if next_location ~= 0
                if sum(state_array==(next_state+1))>0
                    ...
                    if next_location > length(type_idx_array) || next_state > length(beta_array_cell{type_idx_array(next_location)})
                        disp('Indexing Error: next_location/next_state. Program paused, Press Ctrl-C to exit!');
                        pause;
                    end
                    transition_array(ribo+1) = beta_array_cell{type_idx_array(next_location)}(next_state);
                end
    ...
    if protein_diluted == 0 && ribo_movement==1
        if location_array(ribo)==0 && state_array(ribo) == 2
            disp('Logical Error: State 2 with Location 0. Program paused, Press Ctrl-C to exit!');
            pause;
        end
    end
