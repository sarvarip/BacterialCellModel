...
temp = [1*ones(1,187), 1*ones(1,278), 1, 1*ones(1,3506)]; 
...
type_idx_array = [ones(1,total_transcript(1)), 2*ones(1,total_transcript(2)), 3, 4*ones(1,total_transcript(3))]; 
...
for timestep=1:maxsteps
    ...
    if intdev(timestep,2000)==0
        progress = timestep/2000;
        ratio = progress/maxsteps;
        disp([num2str(ratio), '%']);
    end
end
toc
