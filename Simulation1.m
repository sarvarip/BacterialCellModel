...
load('FYP_7_06_endo_gr_newss_ns_100.mat', 'P_count_vec', 'S_i', 'energy', 'location_array', 'state_array', 'temp', 'total_mRNA', 'total_transcript', 'transition_array', 'type_idx_array'); 
energy_init = energy;
...
index_array = [0, 1, 2, 3, 4, 5, 6, 7, 13, 22];
%Mistake fixed: 22 (and 13) must be skipped and not 21

parfor loopvar=1:numel(index_array) %run 1
%parfor index=14:26 %run2
    ...
    ss_start = 36000; %9000 %Times 2 taken out, they should run fast
    ss_end = 40000; %10000
    maxsteps = 10000*ss_end; 
    ref = 10000*ss_start;

    het_init_array = [1/3,1,3];
    het_rel_trc_array = [1/3,1,3];

    codon_choice = zeros(3,60);
    codon_choice(1,:) = max_elongation*ones(1,60);
    codon_choice(2,:) = max_elongation*[ones(1,4), 0.005, ones(1,55)]; %5th is slow
    codon_choice(3,:) = max_elongation*[ones(1,55), 0.005, ones(1,4)]; %56th is slow
    ...
    str = ['LongApril_Sim1_ex_', num2str(index)];
