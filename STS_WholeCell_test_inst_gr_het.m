...
betas{4} = [init_rates(4), max_elongation*ones(1,30)]; %H
...
for timestep=1:maxsteps
    if timestep == 1
        [state_array, location_array, type_idx_array, total_transcript, energy, S_i, time(timestep+1), time_P_cell, P_count_vec, temp, transition_array] = Prod_Rate_Multi_WholeCell_2_final_heterologous_850(state_array, location_array, betas, type_idx_array, total_transcript, energy, S_i, time(timestep), time_P_cell, P_count_vec, temp);
        %reconstruct the transcripts in case changes happened
    elseif timestep == ref+1 
        [state_array, location_array, type_idx_array, total_transcript, energy, S_i, time(timestep+1), time_P_cell, P_count_vec, temp, transition_array] = Prod_Rate_Multi_WholeCell_2_final_heterologous_850(state_array, location_array, betas, type_idx_array, total_transcript, energy, S_i, time(timestep), time_P_cell, P_count_vec, temp, transition_array);
        tot_mass_ref = [7500, 300, 300 300]*P_count_vec';
    elseif timestep == maxsteps
        [state_array, location_array, type_idx_array, total_transcript, energy, S_i, time(timestep+1), time_P_cell, P_count_vec, temp, transition_array] = Prod_Rate_Multi_WholeCell_2_final_heterologous_850(state_array, location_array, betas, type_idx_array, total_transcript, energy, S_i, time(timestep), time_P_cell, P_count_vec, temp, transition_array);
        tot_mass_end = [7500, 300, 300 300]*P_count_vec';
    else    
        [state_array, location_array, type_idx_array, total_transcript, energy, S_i, time(timestep+1), time_P_cell, P_count_vec, temp, transition_array] = Prod_Rate_Multi_WholeCell_2_final_heterologous_850(state_array, location_array, betas, type_idx_array, total_transcript, energy, S_i, time(timestep), time_P_cell, P_count_vec, temp, transition_array);
        %reconstruct the transcripts in case changes happened
    end
    ...
    if rem(timestep,2000)==0
        if timestep >= ref+1
            if exists_reference == 0
                ...
                mass_ref = [7500, 300, 300 300]*P_count_vec';
                exists_reference = 1;
            else
                time_current = time(timestep+1);
                time_elapsed = time_current - time_ref;
                mass_current = [7500, 300, 300 300]*P_count_vec';
